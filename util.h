...
namespace sc2 {
	...
	enum class AGENT_TYPE {
			...
			//zerg 12xx
			ZERG_ANY = 1200,
			ZERG_ANY_BANELING = 1201,
				ZERG_ANY_CHANGELING = 1202,
				ZERG_ANY_CREEPTUMOR = 1203,
				ZERG_ANY_DRONE = 1204,
				ZERG_ANY_OVERLORD = 1205,

			ZERG_ANY_MILITARY = 1220,
			ZERG_ANY_CIVIL = 1221,
			ZERG_ANY_BUILDING = 1222,
			ZERG_ANY_COCOON = 1223,
				
				ZERG_ANY_TOWN_HALL = 1230,

			//terran 13xx
			TERRAN_ANY = 1300,
			TERRAN_ANY_SUPPLYDEPOT = 1301,
			TERRAN_ANY_BARRACKS = 1302,
			TERRAN_ANY_FACTORY = 1303,
			TERRAN_ANY_STARPORT = 1304,
			TERRAN_ANY_HELLION = 1305,
			TERRAN_ANY_LIBERATOR = 1306,
			TERRAN_ANY_SIEGETANK = 1307,
			TERRAN_ANY_THOR = 1308,
			TERRAN_ANY_VIKINK = 1309,
			TERRAN_ANY_WIDOMINE = 1310,

			TERRAN_ANY_MILITARY = 1320,
			TERRAN_ANY_CIVIL = 1321,
			TERRAN_ANY_BUILDING = 1322,

				TERRAN_ANY_TOWN_HALL = 1330,

			//protoss 14xx
			PROTOSS_ANY = 1400,
			PROTOSS_ANY_ADEPT = 1401,
			PROTOSS_ANY_DISRUPTOR = 1402,
			PROTOSS_ANY_ORACLE = 1403,
			PROTOSS_ANY_WARPPRISM = 1404,

			PROTOSS_ANY_MILITARY = 1420,
			PROTOSS_ANY_CIVIL = 1421,
			PROTOSS_ANY_BUILDING = 1422,
	};

	//smart unit filter
	struct IsAgent {
		IsAgent(AGENT_TYPE type, const ObservationInterface* obs) : type_(type), obs_(obs) {};
		AGENT_TYPE type_;
		UnitTypeData data_;
		const ObservationInterface* obs_;
		bool operator()(const Unit& unit) { 
			switch (type_)
			{
			case AGENT_TYPE::ALL_FLYING:
				return unit.is_flying;
			case AGENT_TYPE::ALL_TOWN_HALL:
				return IsAgent(AGENT_TYPE::ZERG_ANY_TOWN_HALL, obs_)(unit)
					|| IsAgent(AGENT_TYPE::TERRAN_ANY_TOWN_HALL, obs_)(unit)
					|| IsAgent(AGENT_TYPE::PROTOSS_NEXUS, obs_)(unit);
			case AGENT_TYPE::ALL_BUILDING:
				data_ = obs_->GetUnitTypeData().at(unit.unit_type);

				for (const auto& attribute : data_.attributes) {
			...
			case AGENT_TYPE::TERRAN_ANY_FACTORY:
			...
			case AGENT_TYPE::TERRAN_ANY_STARPORT:
			...
			case AGENT_TYPE::TERRAN_ANY_HELLION:
				return unit.unit_type == UNIT_TYPEID::TERRAN_HELLION
					|| unit.unit_type == UNIT_TYPEID::TERRAN_HELLIONTANK;
			case AGENT_TYPE::TERRAN_ANY_LIBERATOR:
				return unit.unit_type == UNIT_TYPEID::TERRAN_LIBERATOR
					|| unit.unit_type == UNIT_TYPEID::TERRAN_LIBERATORAG;
			case AGENT_TYPE::TERRAN_ANY_SIEGETANK:
				return unit.unit_type == UNIT_TYPEID::TERRAN_SIEGETANK
					|| unit.unit_type == UNIT_TYPEID::TERRAN_SIEGETANKSIEGED;
			case AGENT_TYPE::TERRAN_ANY_MILITARY:
				return (IsAgent(AGENT_TYPE::TERRAN_ANY, obs_)(unit)
					&& !IsAgent(AGENT_TYPE::TERRAN_ANY_CIVIL, obs_)(unit)
					&& !IsAgent(AGENT_TYPE::TERRAN_ANY_BUILDING, obs_)(unit))
					|| unit.unit_type == UNIT_TYPEID::TERRAN_AUTOTURRET
					|| unit.unit_type == UNIT_TYPEID::TERRAN_MISSILETURRET
					|| unit.unit_type == UNIT_TYPEID::TERRAN_BUNKER
					|| unit.unit_type == UNIT_TYPEID::TERRAN_PLANETARYFORTRESS;
			case AGENT_TYPE::TERRAN_ANY_CIVIL:
				return unit.unit_type == UNIT_TYPEID::TERRAN_MULE
					|| unit.unit_type == UNIT_TYPEID::TERRAN_SCV;
			case AGENT_TYPE::TERRAN_ANY:
				return obs_->GetUnitTypeData().at(unit.unit_type).race == Race::Terran;
			case AGENT_TYPE::TERRAN_ANY_BUILDING:
				return IsAgent(AGENT_TYPE::ALL_BUILDING, obs_)(unit)
					&& IsAgent(AGENT_TYPE::TERRAN_ANY, obs_)(unit);
			case AGENT_TYPE::ZERG_ANY:
				return obs_->GetUnitTypeData().at(unit.unit_type).race == Race::Zerg;
			case AGENT_TYPE::ZERG_ANY_BUILDING:
				return IsAgent(AGENT_TYPE::ALL_BUILDING, obs_)(unit)
					&& IsAgent(AGENT_TYPE::ZERG_ANY, obs_)(unit);
			case AGENT_TYPE::PROTOSS_ANY:
				return obs_->GetUnitTypeData().at(unit.unit_type).race == Race::Protoss;
			case AGENT_TYPE::PROTOSS_ANY_BUILDING:
				return IsAgent(AGENT_TYPE::ALL_BUILDING, obs_)(unit)
					&& IsAgent(AGENT_TYPE::PROTOSS_ANY, obs_)(unit); 
			case AGENT_TYPE::PROTOSS_ANY_ADEPT:
				return unit.unit_type == UNIT_TYPEID::PROTOSS_ADEPT
					|| unit.unit_type == UNIT_TYPEID::PROTOSS_ADEPTPHASESHIFT;
			case AGENT_TYPE::PROTOSS_ANY_DISRUPTOR:
				return unit.unit_type == UNIT_TYPEID::PROTOSS_DISRUPTOR
					|| unit.unit_type == UNIT_TYPEID::PROTOSS_DISRUPTORPHASED;
			case AGENT_TYPE::PROTOSS_ANY_ORACLE:
				return unit.unit_type == UNIT_TYPEID::PROTOSS_ORACLE
					|| unit.unit_type == UNIT_TYPEID::PROTOSS_ORACLESTASISTRAP;
			case AGENT_TYPE::PROTOSS_ANY_WARPPRISM:
				return unit.unit_type == UNIT_TYPEID::PROTOSS_WARPPRISM
					|| unit.unit_type == UNIT_TYPEID::PROTOSS_WARPPRISMPHASING;
			case AGENT_TYPE::PROTOSS_ANY_CIVIL:
				return unit.unit_type == UNIT_TYPEID::PROTOSS_OBSERVER
					|| unit.unit_type == UNIT_TYPEID::PROTOSS_PROBE
					|| IsAgent(AGENT_TYPE::PROTOSS_ANY_WARPPRISM, obs_)(unit);
			case AGENT_TYPE::PROTOSS_ANY_MILITARY:
				return (IsAgent(AGENT_TYPE::PROTOSS_ANY, obs_)(unit)
					&& !IsAgent(AGENT_TYPE::PROTOSS_ANY_CIVIL, obs_)(unit)
					&& !IsAgent(AGENT_TYPE::PROTOSS_ANY_BUILDING, obs_)(unit))
					|| unit.unit_type == UNIT_TYPEID::PROTOSS_PHOTONCANNON
					|| unit.unit_type == UNIT_TYPEID::PROTOSS_PYLONOVERCHARGED;
			case AGENT_TYPE::ZERG_ANY_BANELING:
				return unit.unit_type == UNIT_TYPEID::ZERG_BANELING
					|| unit.unit_type == UNIT_TYPEID::ZERG_BANELINGBURROWED;
			case AGENT_TYPE::ZERG_ANY_DRONE:
				return unit.unit_type == UNIT_TYPEID::ZERG_DRONE
					|| unit.unit_type == UNIT_TYPEID::ZERG_DRONEBURROWED;
			case AGENT_TYPE::ZERG_ANY_CREEPTUMOR:
				return unit.unit_type == UNIT_TYPEID::ZERG_CREEPTUMOR
					|| unit.unit_type == UNIT_TYPEID::ZERG_CREEPTUMORBURROWED
					|| unit.unit_type == UNIT_TYPEID::ZERG_CREEPTUMORQUEEN;
			case AGENT_TYPE::ZERG_ANY_CHANGELING:
				return unit.unit_type == UNIT_TYPEID::ZERG_CHANGELING
					|| unit.unit_type == UNIT_TYPEID::ZERG_CHANGELINGMARINE
					|| unit.unit_type == UNIT_TYPEID::ZERG_CHANGELINGMARINESHIELD
					|| unit.unit_type == UNIT_TYPEID::ZERG_CHANGELINGZEALOT
					|| unit.unit_type == UNIT_TYPEID::ZERG_CHANGELINGZERGLING
					|| unit.unit_type == UNIT_TYPEID::ZERG_CHANGELINGZERGLINGWINGS;
			case AGENT_TYPE::ZERG_ANY_COCOON:
				return unit.unit_type == UNIT_TYPEID::ZERG_BANELINGCOCOON
					|| unit.unit_type == UNIT_TYPEID::ZERG_BROODLORDCOCOON
					|| unit.unit_type == UNIT_TYPEID::ZERG_OVERLORDCOCOON
					|| unit.unit_type == UNIT_TYPEID::ZERG_BANELINGBURROWED
					|| unit.unit_type == UNIT_TYPEID::ZERG_BANELINGBURROWED
					|| unit.unit_type == UNIT_TYPEID::ZERG_BANELINGBURROWED
					|| unit.unit_type == UNIT_TYPEID::ZERG_BANELINGBURROWED
					...
					|| unit.unit_type == UNIT_TYPEID::ZERG_BANELINGBURROWED
					;
			case AGENT_TYPE::ZERG_ANY_CIVIL:
				return IsAgent(AGENT_TYPE::ZERG_ANY_COCOON, obs_)(unit)
					|| IsAgent(AGENT_TYPE::ZERG_ANY_CHANGELING, obs_)(unit)
					|| IsAgent(AGENT_TYPE::ZERG_ANY_DRONE, obs_)(unit)
					|| unit.unit_type == UNIT_TYPEID::ZERG_EGG
					|| unit.unit_type == UNIT_TYPEID::ZERG_INFESTEDTERRANSEGG
					|| unit.unit_type == UNIT_TYPEID::ZERG_LARVA
					|| unit.unit_type == UNIT_TYPEID::ZERG_LURKERMPEGG
					|| IsAgent(AGENT_TYPE::ZERG_ANY_OVERLORD, obs_)(unit);
			case AGENT_TYPE::ZERG_ANY_MILITARY:
				return (IsAgent(AGENT_TYPE::ZERG_ANY, obs_)(unit)
					&& !IsAgent(AGENT_TYPE::ZERG_ANY_BUILDING, obs_)(unit)
					&& !IsAgent(AGENT_TYPE::ZERG_ANY_CIVIL, obs_)(unit))
					|| unit.unit_type == UNIT_TYPEID::ZERG_SPINECRAWLER
					|| unit.unit_type == UNIT_TYPEID::ZERG_SPORECRAWLER;
			default:
				return unit.unit_type == (int)type_;
			}
		};
	};
	...
	public:
		...
		int CountAgentType(Unit::Alliance alliance, AGENT_TYPE agentType);
		int CountSelfAgentType(AGENT_TYPE agentType);

		bool HasResourcesToBuild(UNIT_TYPEID typeId);

		bool IsIdle();
		bool IsIdle(const Unit* unit);
		bool IsIdle(Tag tag);

		int CountOrdersType(ABILITY_ID abilityId, Unit::Alliance alliance);
		int CountSelfOrdersType(ABILITY_ID abilityId);
